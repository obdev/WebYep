$(document).ready(function(){
    ...
    WY.menu = {
        ...
        buildDOM: function(rootElement) { // at this point we have a correct model
            ...
            $('.WY_EditorMenuItem').bind('click', function(){
                //Added by AKO
                var _externalMenuOdl = '';
                $('#WY_EditMenuExternal').prop('checked',false);
                ...
                $('#WY_EditMenuExternal').attr('checked', WY.menu.getSelectedItem().link.indexOf("?DOC_INST=0") >= 0  ? 'checked' : false);
    ...
    var _externalMenuOdl = '';
    //Checkbox external menu
    $('#WY_EditMenuExternal').click(function () {
        if($.trim($('#WY_EditMenuLink').val())!=''){
            _externalMenuOdl = $('#WY_EditMenuLink').val();

            var arr = ["asp","aspx","axd","asx","asmx","ashx","css","cfm","yaws","swf","html","htm","xhtml","jhtml","Java","jsp","jspx","wss","do","action","js","pl","php","php4","php3","phtml","py","rb","rhtml","xml","rss","svg","cgi","dll"];
            var _extension = (/[.]/.exec(_externalMenuOdl)) ? /[^.]+$/.exec(_externalMenuOdl) : undefined;
            if($('#WY_EditMenuExternal').is( ":checked" )){
                if(jQuery.inArray( _extension, arr )){
                    $('#WY_EditMenuLink').val(_externalMenuOdl+'?DOC_INST=0')
                }else if (!_externalMenuOdl.match(/\/$/)) {
                    $('#WY_EditMenuLink').val(_externalMenuOdl+'/?DOC_INST=0')
                }else{
                    $('#WY_EditMenuLink').val(_externalMenuOdl+'?DOC_INST=0')
                }
            }else{
                if(WY.menu.getSelectedItem().link.indexOf("?DOC_INST=0") >= 0)
                    _externalMenuOdl = _externalMenuOdl.replace('/?DOC_INST=0','');
                    _externalMenuOdl = _externalMenuOdl.replace('?DOC_INST=0','');
                $('#WY_EditMenuLink').val(_externalMenuOdl)
            }
            $( "#WY_EditMenuLink" ).change();
        }
    });
