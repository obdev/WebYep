...
class WYDocument
{
	...
	{
		$oReturn = od_nil;
		$oP = (new WYDocument())->oDocumentsFilePath();
		$oF = new WYFile($oP);
		if ($oF->bExists()) {
			$aLines = $oF->aContentLines();
		}
		foreach ($aLines as $sLine) {
			$dEntry = (new WYDocument())->dParseDocumentsFileLine($sLine);
			if ($dEntry['id'] == $iID) {
	...
	function setLoopID($i)
	{
        ...
        (new WYLoopElement())->setCurrentLoopID($i);
	}
	...
	function iLoopID11()
	{
		...
		return (new WYLoopElement())->iCurrentLoopID();
...
function iLoopID()
	...
	if(isset($_REQUEST['DOC_INST']) && !empty($_REQUEST['DOC_INST'])){
	return (new WYLoopElement())->iCurrentLoopID(); 
