...
{
	$s = (new WYLanguage())->sTranslatedString($s, $bOutputAsHTML);
	return $s;
}
...
function sWYTDate($i)
{
   global $webyep_iLanguageID, $webyep_CustomDateFromat;

   return $webyep_iLanguageID == $webyep_CustomDateFromat ? date($webyep_CustomDateFromat, $i):date("Y-m-d", $i);
}

function sWYTTime($i)
{
   global $webyep_iLanguageID, $webyep_CustomTimeFromat;

   return $webyep_iLanguageID == $webyep_CustomTimeFromat ? date($webyep_CustomTimeFromat, $i):date("h:i:s a", $i);
}
...
class WYLanguage
{
	function setup()
	{
      ...
      if ($webyep_sLang == "" || $webyep_sLang == "auto") {
         ...
         if (strstr((new WYApplication())->sScriptPath(__FILE__), "/programm/lib/WYLanguage")) $webyep_iLanguageID = WYLANG_GERMAN;
         else if (strstr((new WYApplication)->sScriptPath(__FILE__), "/program/lib/WYLanguage")) $webyep_iLanguageID = WYLANG_ENGLISH;
         else $goApp->fatalError("Could not determine language");
      }
      else if ($webyep_sLang == "english") {
         $webyep_iLanguageID = WYLANG_ENGLISH;
      }
      else if ($webyep_sLang == "deutsch") {
